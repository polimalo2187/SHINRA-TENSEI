...
from app.oanda_api import get_candles_df, get_scan_instruments, is_market_open
...
MARKET_CLOSED_SLEEP_SECONDS = int(os.getenv("FOREX_MARKET_CLOSED_SLEEP_SECONDS", "300"))
...
def get_klines(symbol: str, interval: str, limit: int = 220) -> pd.DataFrame:
    ...
    granularity = granularity_map.get(interval.lower(), interval.upper())
    df = get_candles_df(symbol, granularity, count=limit)
    if df.empty:
...
def get_active_futures_symbols() -> List[str]:
    # Mantengo el nombre por compatibilidad con el resto del proyecto,
    # aunque aquí ya devolvemos instrumentos Forex/Metals de OANDA.
    instruments = get_scan_instruments()
    logger.info("📊 %s instrumentos activos para scanner Forex", len(instruments))
    return instruments
...
def recent_duplicate_exists(symbol: str, direction: str, visibility: str) -> bool:
    ...
    if exists:
        logger.info("♻️ Duplicado reciente detectado: %s %s (%s)", symbol, direction, visibility)
    return exists
...
def _pick_best(pool: List[Dict], predicate, used_symbols: set[str]) -> Optional[Dict]:
    for signal in pool:
...
async def scan_market_async(bot: Bot):
    logger.info("📡 Scanner Forex iniciado — raw_score real + clasificación por plan")

    while True:
        try:
            if not is_market_open():
                logger.info("🌙 Mercado Forex cerrado. Scanner en pausa segura.")
                await asyncio.sleep(MARKET_CLOSED_SLEEP_SECONDS)
                continue
            ...
            for symbol in symbols:
                try:
                    ...
                    result = mtf_strategy(df_1h, df_15m, df_5m, symbol=symbol)
                    if result:
                    ...
                    logger.debug("⚠️ Error procesando %s: %s", symbol, exc)
            ...
            for visibility, medal, signal in selected:
                ...
                try:
                    ...
                    logger.error("⚠️ Error notificando señal: %s", exc)
