...
from app.oanda_api import format_price, get_market_rows, get_radar_opportunities, get_top_movers
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "register_exchange":
            ...
            await query.edit_message_text(
                "🌐 Envía el nombre de tu broker o feed (ej: OANDA, FXCM, IG):"
            )
            return
...
async def handle_watchlist_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    para añadir (ej: EURUSD, GBP_USD, XAUUSD, EURUSD,GBPUSD,XAUUSD).
    """
    try:
        ...
        if not symbols:
            await msg.reply_text("❌ Símbolo inválido. Ej: EURUSD o XAUUSD")
            return
        ...
        prefix = ((last_res[1] if isinstance(last_res, tuple) else str(last_res)) + "\n\n") if last_res else ""
...
async def handle_market(query, user):
    """Panel de mercado Forex/Metals: gainers, losers, mayor actividad y mayor ATR."""
    try:
        rows = get_market_rows()
    except Exception as e:
        logger.exception("Error obteniendo mercado Forex: %s", e)
        await query.edit_message_text(
    ...
    highest_activity = sorted(rows, key=lambda x: float(x.get("quoteVolume", 0.0)), reverse=True)[:5]
    highest_atr = sorted(rows, key=lambda x: float(x.get("atr_pct", 0.0)), reverse=True)[:5]
    ...
    for row in gainers:
        lines.append(f"• *{row['symbol']}* — {float(row['priceChangePercent']):+.2f}%")

    lines.extend(["", "💀 *Top Losers 24h*"])
    for row in losers:
        lines.append(f"• *{row['symbol']}* — {float(row['priceChangePercent']):+.2f}%")

    lines.extend(["", "💰 *Mayor actividad 24h*"])
    for row in highest_activity:
        lines.append(f"• *{row['symbol']}* — `{float(row['quoteVolume']):,.0f}` ticks")

    lines.extend(["", "⚡ *Mayor volatilidad (ATR 1h)*"])
    for row in highest_atr:
        lines.append(f"• *{row['symbol']}* — `{float(row['atr_pct']) * 100:.3f}%`")
...
async def handle_movers(query, user):
    """Muestra Top Movers 24h de Forex/Metals."""
    try:
        movers = get_top_movers(limit=10)
    except Exception as e:
        logger.exception("Error obteniendo movers Forex: %s", e)
        await query.edit_message_text(
    ...
    lines = ["🔥 TOP MOVERS FOREX (24h)", "", "*OANDA Forex / Metals*", f"🕒 Actualizado: {datetime.utcnow():%H:%M:%S} UTC", ""]

    for i, row in enumerate(movers, start=1):
        symbol = str(row.get("symbol", ""))
        change = float(row.get("priceChangePercent", 0.0))
        activity = float(row.get("quoteVolume", 0.0))
        last_price = float(row.get("lastPrice", 0.0))
        lines.append(f"{i}. *{symbol}*  —  {change:+.2f}%")
        if last_price > 0:
            lines.append(f"   Precio: `{format_price(last_price, symbol)}`")
        if activity > 0:
            lines.append(f"   Actividad 24h: `{activity:,.0f}` ticks")
        lines.append("")
    ...
    await query.edit_message_text("\n".join(lines).strip(), reply_markup=keyboard, parse_mode="Markdown")
...
async def handle_radar(query, user, plan: str):
    ...
    lines = ["📡 RADAR FOREX", "", "Top oportunidades Forex:", ""]

    for idx, item in enumerate(opportunities, start=1):
        symbol = item["symbol"]
        score = int(item["score"])
        change_24h = float(item.get("change_pct", 0.0))
        change_1h = float(item.get("change_1h", 0.0))
        atr_pct = float(item.get("atr_pct", 0.0)) * 100.0
        activity = float(item.get("quote_volume", 0.0))
        direction = item["direction"]
        trend = item.get("trend", "Mixta")
        momentum = item.get("momentum", "Medio")

        lines.append(f"{idx}️⃣ {symbol} — {direction}")
        lines.append(f"Score: {score} | 24h: {change_24h:+.2f}% | 1h: {change_1h:+.2f}%")
        lines.append(f"Actividad 24h: {activity:,.0f} | Tendencia: {trend}")

        if plan == PLAN_PREMIUM:
            lines.append(f"Momentum: {momentum} | ATR 1h: {atr_pct:.3f}%")

        lines.append("")

    keyboard = [[InlineKeyboardButton("🔄 Actualizar", callback_data="radar_refresh"), InlineKeyboardButton("⬅️ Volver", callback_data="back_menu")]]
    try:
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|radar|radar_refresh|performance|reset_stats|movers|market|market_refresh|watchlist|wl_refresh|wl_clear|wl_rm:[A-Z0-9_]+|alerts|alerts_refresh|history|history_refresh|plans|my_account|referrals|support|admin_panel|admin_activate_plan|admin_delete_user|back_menu|choose_plus_plan|choose_premium_plan|register_exchange)$"
        ),
